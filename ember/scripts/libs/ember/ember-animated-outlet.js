define(['ember'], function(Ember) {
	...
	Ember.AnimatedContainerView = Ember.ContainerView.extend({
		...
		init : function() {
			this._super();
			// Register this view, so queued effects can be related with this
			// view by name
		...
		_currentViewWillChange : Ember.beforeObserver(function() {
			...
			if (currentView) {
				// Store the old `currentView` (and don't destroy it yet) so we
				// can use it for animation later
				this.set('oldView', currentView);
			}
		}, 'currentView'),

		_currentViewDidChange : Ember.observer(function() {
			var newView = Ember.get(this, 'currentView'), oldView = Ember.get(this, 'oldView'), name = this.get('name'), effect = null;
			if (newView) {
				if (oldView) {
				...
				if (this._queuedAnimation) {
					oldView.destroy(); // the oldView has never been visible,
										// and never will be, so we can just
										// destroy it now
					oldView = this._queuedAnimation.oldView; // instead, use
																// the oldView
																// from the
																// queued
																// animation,
		...
		_handleAnimationQueue : function() {
			// If animation is in progress, just stop here. Once the animation
			// has finished, this method will be called again.
			if (this._isAnimating) {
				return;
			}
			var self = this, q = this._queuedAnimation;
			if (q) {
				var newView = q.newView, oldView = q.oldView, effect = q.effect;
				this._queuedAnimation = null;
				// Push the newView to this view, which will append it to the
				// DOM
				this.pushObject(newView);
				if (oldView && effect) {
					// If an effect is queued, then start the effect when the
					// new view has been inserted in the DOM
					...
					newView.on('didInsertElement', function() {
						Ember.AnimatedContainerView._effects[effect](self, newView, oldView, function() {
							Em.run(function() {
								...
								oldView.destroy();
								// Check to see if there are any queued
								// animations
					...
					if (oldView) {
						// If there is no effect queued, then just remove the
						// old view (as would normally happen in a
						// ContainerView)
	...
	Ember.AnimatedContainerView.reopenClass({
		...
		/**
		 * Whenever an animated route transition is set in motion, it will be
		 * stored here, so the animated outlet view can pick it up
		 * 
		 * @private
		...
		/**
		 * Enqueue effects to be executed by the given outlets when the next
		 * route transition happens.
		 * 
		 * @param {Object}
		 *            animations A hash with keys corresponding to outlet views
		 *            and values with the desired animation effect.
		 */
		...
		/**
		 * All animation effects are stored on this object and can be referred
		 * to by its key
		 * 
		 * @private
		 ...
		 *  - The `Ember.AnimatedContainerView` instance. - The new view. - The
		 * old view.
		 * 
		 * @param {String}
		 *            effect The name of the effect, e.g. 'slideLeft'
		 * @param {Function}
		 *            callback The function to call when effect has to be
		 *            executed
		 */
	...
	Ember.onLoad('Ember.Handlebars', function(Handlebars) {
		...
		function args(linkView, router, route) {
			var ret = get(linkView, 'routeArgs').slice(), animations = linkView.parameters.animations;
		...
		var AnimatedLinkView = Ember.AnimatedLinkView = Ember.LinkView.extend({
			_invoke : function(event) {
				...
				var router = this.get('router'), routeArgs = args(this, router);
	...
	Ember.Router.reopen({
		...
		/**
		 * Works as {@link Ember.Router.transitionTo}} except that it takes a
		 * third parameter, `animations`, which will enqueue animations.
		 * 
		 * `animations` should be an object with outlet names as keys and effect
		 * names as value.
		 * 
		 * @param name
		 * @param animations
		 *            {Object} Animations to enqueue
		 * @param model
		 */
		...
		/**
		 * Works as {@link Ember.Router.replaceWith}} except that it takes a
		 * third parameter, `animations`, which will enqueue animations.
		 * 
		 * `animations` should be an object with outlet names as keys and effect
		 * names as value.
	...
	Ember.ControllerMixin.reopen({
		...
		/**
		 * Works as {@link Ember.ControllerMixin.transitionToRoute}} except
		 * that it takes a third parameter, `animations`, which will enqueue
		 * animations.
		 * 
		 * `animations` should be an object with outlet names as keys and effect
		 * names as value.
		...
		/**
		 * Works as {@link Ember.ControllerMixin.replaceRoute}} except that it
		 * takes a third parameter, `animations`, which will enqueue animations.
		 * 
		 * `animations` should be an object with outlet names as keys and effect
		 * names as value.
		 * 
	...
	Ember.AnimatedContainerView.registerEffect('fade', function(ct, newView, oldView, callback) {
		var newEl = newView.$(), oldEl = oldView.$();
	...
	Ember.AnimatedContainerView.registerEffect('flip', function(ct, newView, oldView, callback) {
		var ctEl = ct.$(), newEl = newView.$(), oldEl = oldView.$();
	...
	(function() {
		...
		var slide = function(ct, newView, oldView, callback, direction, slow) {
			var ctEl = ct.$(), newEl = newView.$(), duration = slow ? 2050 : 450;
	...
	(function() {
		...
		var slideOver = function(ct, newView, oldView, callback, direction) {
			var ctEl = ct.$(), newEl = newView.$(), duration = 450;
