define([ 'app',
         'tpl!apps/affinity/view.tpl',
         'css!apps/affinity/style', 'apps/affinity/affinity_propagation',
         'apps/affinity/graphics',
         'apps/affinity/controller'], function(App, viewTpl) {
	App.module('affinity', function(affinity, App, Backbone, Marionette, $, _) {
		
		var _ap_;
		var _interval_;

		affinity.MainView = Marionette.ItemView.extend({
			template : viewTpl,
			onShow: function(){
				var points = [ [ 12, 12 ], [ 250, 40 ], [ 200, 200 ], [ 388, 288 ], [ 100, 30 ], [ 50, 50 ], [ 20, 250 ], [ 340, 120 ], [ 300, 200 ], [ 340, 80 ], [ 40, 288 ], [ 50, 140 ], [ 100, 200 ]
				/*
				 * [570,570], [500, 230], [780, 520], [725, 10], [510, 50], [10, 500], [20,
				 * 330], [120, 450], [230, 500], [60, 234], [400,400], [500, 340], [559, 490],
				 * [730, 320], [700, 545]
				 */
				];
				...
				_interval_ = setInterval(drawMovingParticles.draw, 10);
		...
		affinity.controller = {
			show : function() {
				App.navigate("affinity");
				App.mainRegion.show(new affinity.MainView());
			}
		}

		App.on("affinity:show", function() {
			affinity.controller.show();
		});

		affinity.Router = Marionette.AppRouter.extend({
			appRoutes : {
				"affinity" : "show"
			}
		});
		...
		App.addInitializer(function() {
			new affinity.Router({
				controller : affinity.controller
			});
		});
	});
	return App.affinity.controller;
})