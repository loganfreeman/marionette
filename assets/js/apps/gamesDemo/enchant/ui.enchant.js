define([ 'enchant' ], function(enchant) {
	/**
	 * @fileOverview ui.enchant.js v2 (2012/11/05) ui parts support
	 * @require enchant.js v0.5.2 or later
	 * @require image files for gamepad, icons (default: pad.png, apad.png,
	 *          icon0.png, font0.png)
	 * 
	 * @features - D-Pad (left, right, up, down) - Analog Pad - Button (3
	 *           built-in themes and can be customized) - MutableText -
	 *           ScoreLabel - TimeLabel - LifeLabel - Bar - VirtualMap
	 * 
	 * @usage [D-Pad] var pad = new Pad(); pad.x = 0; pad.y = 220;
	 *        core.rootScene.addChild(pad); (input of X direction can be
	 *        detected from "Xbuttonup" "Xbuttondown" events or
	 *        enchant.Core.instance.input.X)
	 * 
	 * [A-Pad] var pad = new APad(); pad.x = 0; pad.y = 220;
	 * core.rootScene.addChild(pad); (input can be detected from pad.vx/vy and
	 * pad.touched)
	 * 
	 * [Button] var button = new Button("Press me");
	 ...
	 */
	enchant.ui = {
		assets : [ 'assets/js/apps/gamesDemo/enchant/images/pad.png', 
		           'assets/js/apps/gamesDemo/enchant/images/apad.png', 
		           'assets/js/apps/gamesDemo/enchant/images/icon0.png', 
		           'assets/js/apps/gamesDemo/enchant/images/font0.png' ]
	};
	...
	enchant.ui.Pad = enchant.Class.create(enchant.Sprite, {
		...
		initialize : function() {
			...
			var image = core.assets['assets/js/apps/gamesDemo/enchant/images/pad.png'];
	...
	enchant.ui.APad = enchant.Class.create(enchant.Group, {
		 ...
		 * @param mode
		 *            'direct': 入力ベクトルは正規化されない (大きさは 0~1 の間) 'normal':
		 *            入力ベクトルを常に正規化する (大きさは常に1となる)
		 */
		initialize : function(mode) {
			var core = enchant.Core.instance;
			var image = core.assets['assets/js/apps/gamesDemo/enchant/images/apad.png'];
			 ...
			 * vx, vy
			 * 
			 * @type {Number} 入力ベクトルの(x, y)方向の大きさ
			 */
			this.vx = 0;
			this.vy = 0;
			 ...
			 * rad, dist
			 * 
			 * @type {Number} 入力ベクトルの極座標表示 radは角度、distはベクトルの大きさを示す
			 */
			this.rad = 0;
			this.dist = 0;
	...
	/**
	 * ボタンオブジェクトのクラス: Button available in only DOMGroup
	 * 
	...
	enchant.ui.Button = enchant.Class.create(enchant.Entity, {
		...
		/**
		 * Text color settings. CSS 'color' can be set to same format as
		 * properties.
		 * 
		 * @type {String}
		 */
	...
	enchant.ui.MutableText = enchant.Class.create(enchant.Sprite, {
		/**
		 * [lang:ja] ビットマップフォントを用いたラベルクラス (参考: draw.text.js
		 * http://d.hatena.ne.jp/nakamura001/20110430/1304181043) enchant.js
		 * 添付素材の font*.png が利用可能。 [/lang] [lang:en] [/lang]
		 * 
		 * @usage var text = new MutableText(0, 0); game.text = 'Hello, world!';
		...
		setText : function(txt) {
			...
			// if image is to small or was to big for a long time create new
			// image
			...
			for (i = 0; i < txt.length; i++) {
				...
				this.image.draw(enchant.Game.instance.assets['assets/js/apps/gamesDemo/enchant/images/font0.png'], x * this.fontSize, y * this.fontSize, this.fontSize, this.fontSize, (i % this.row) * this.fontSize, ((i / this.row) | 0) * this.fontSize, this.fontSize, this.fontSize);
			}
		},
		/**
		 * ラベルの内容
	...
	enchant.ui.ScoreLabel = enchant.Class.create(enchant.ui.MutableText, {
		/**
		 * スコアを表示するラベル。 画像フォントクラス (MutableText) を使って表示する。
		 * 
		 * @constructs
		 * @param x
		 * @param y
	...
	enchant.ui.LifeLabel = enchant.Class.create(enchant.Group, {
		/**
		 * ライフを表示する専用のラベル icon0.png 内のハートの画像を用いる
		 * 
		 * @constructs
		 * @param x
		 * @param y
		...
		initialize : function(x, y, maxlife) {
			...
			for ( var i = 0; i < this._maxlife; i++) {
				...
				this.heart[i].image = enchant.Game.instance.assets['assets/js/apps/gamesDemo/enchant/images/icon0.png'];
	...
	enchant.ui.VirtualMap = enchant.Class.create(enchant.Group, {
		/**
		 * マップライクな Group addChildで Sprite 等を追加すると、自動的に mx, my プロパティが追加され、
		 * VirtualMap内での座標で Sprite を操作できる
		 * 
		 * 使い方 //20 x 20 メッシュの縦横320ピクセルの盤を作り、その上に16 x 16の駒を8つ並べる var board = new
		 * VirtualMap(20, 20); board.width = 320; board.height = 320; for(var
		 * i=0; i<8; i++){ var piece = new Sprite(16, 16); piece.image =
		 * game.assets['icon0.gif']; board.addChild(piece); piece.mx = i + 3;
		 * piece.my = 16; } game.rootScene.addChild(board);
		 * 
		 * @param meshWidth
		 * @param meshHeight
		 * @constructs
		...
		/**
		 * オブジェクトを VirtualMap にバインドする。 バインドされたオブジェクトはメッシュ座標 mx, my
		 * プロパティを持ち、これを操作することで VirtualMap の中を移動させることができる。
		 * 
		 * @param obj
		 */
		bind : function(obj) {
