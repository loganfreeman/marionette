		...
		function(Matter) {
			var run = (function() {
				...
				Demo.init = function() {
					...
					// NOTE: this is actually Matter.Engine.create(), see the
					// aliases at top of this file
					...
					_sceneName = _sceneName.split("/").pop();
				...
				Demo.mixed = function() {
					...
					var stack = Composites.stack(20, 20, 15, 4, 0, 0, function(x, y, column, row) {
						...
						// triangles can be a little unstable, so avoid until
						// fixed
				...
				Demo.rounded = function() {
					...
					World.add(_world, [ Bodies.rectangle(200, 200, 100, 100, {
						chamfer : {
							radius : 20
						}
					}),
				...
				Demo.views = function() {
					...
					// make the world bounds a little bigger than the render
					// bounds
					...
					var boundsScaleTarget = 1, boundsScale = {
						x : 1,
						y : 1
					};

					// use the engine tick event to control our view
					_sceneEvents.push(Events.on(_engine, 'beforeTick', function() {
						var world = _engine.world, mouse = _engine.input.mouse, render = _engine.render, translate;
						...
						var deltaCentre = Vector.sub(mouse.absolute, viewportCentre), centreDist = Vector.magnitude(deltaCentre);

						// translate the view if mouse has moved over 50px from
						// the centre of viewport
						if (centreDist > 50) {
							// create a vector to translate the view, allowing
							// the user to control view speed
							var direction = Vector.normalise(deltaCentre), speed = Math.min(10, Math.pow(centreDist - 50, 2) * 0.0002);
				...
				Demo.chains = function() {
					var _world = _engine.world, groupId = Body.nextGroupId();
				...
				Demo.bridge = function() {
					...
					World.add(_world, [ bridge, Bodies.rectangle(80, 440, 120, 280, {
						isStatic : true
				...
				Demo.car = function() {
					var _world = _engine.world, scale;
					...
					World.add(_world, [ Bodies.rectangle(200, 150, 650, 20, {
						isStatic : true,
						angle : Math.PI * 0.06
				...
				Demo.friction = function() {
					...
					World.add(_world, [ Bodies.rectangle(300, 180, 700, 20, {
						isStatic : true,
						angle : Math.PI * 0.06
					...
					World.add(_world, [ Bodies.rectangle(300, 350, 700, 20, {
						isStatic : true,
						angle : Math.PI * 0.06
					...
					World.add(_world, [ Bodies.rectangle(300, 520, 700, 20, {
						isStatic : true,
						angle : Math.PI * 0.06
				...
				Demo.airFriction = function() {
					...
					World.add(_world, [ Bodies.rectangle(200, 100, 60, 60, {
						frictionAir : 0.001
				...
				Demo.avalanche = function() {
					...
					World.add(_world, [ Bodies.rectangle(200, 150, 700, 20, {
						isStatic : true,
						angle : Math.PI * 0.06
				...
				Demo.timescale = function() {
					...
					var timeScaleTarget = 1, counter = 0;
				...
				Demo.ballPool = function() {
					...
					World.add(_world, [ stack, Bodies.polygon(200, 560, 3, 60), Bodies.polygon(400, 560, 5, 60), Bodies.rectangle(600, 560, 80, 80) ]);
				...
				Demo.restitution = function() {
					...
					var rest = 0.9, space = 600 / 5;

					World.add(_world, [ Bodies.rectangle(100 + space * 0, 150, 50, 50, {
						restitution : rest
				...
				Demo.softBody = function() {
					...
					World.add(_world, [ Composites.softBody(250, 100, 5, 5, 0, 0, true, 18, particleOptions), Composites.softBody(250, 300, 8, 3, 0, 0, true, 15, particleOptions), Composites.softBody(250, 400, 4, 4, 0, 0, true, 15, particleOptions) ]);
				...
				Demo.cloth = function() {
					...
					var groupId = Body.nextGroupId(), particleOptions = {
						friction : 0.00001,
						groupId : groupId,
						render : {
							visible : false
					...
					World.add(_world, [ cloth, Bodies.circle(300, 500, 80, {
						isStatic : true
				...
				Demo.catapult = function() {
					...
					World.add(_world, [ stack, catapult, Bodies.rectangle(250, 555, 20, 50, {
						isStatic : true
				...
				Demo.events = function() {
					...
					Events.on(_engine, 'collisionStart', function(event) {
						...
						// change object colours to show those starting a
						// collision
					...
					Events.on(_engine, 'collisionActive', function(event) {
						...
						// change object colours to show those in an active
						// collision (e.g. resting contact)
						for (var i = 0; i < pairs.length; i++) {
							var pair = pairs[i];
					...
					Events.on(_engine, 'collisionEnd', function(event) {
						...
						// change object colours to show those ending a
						// collision
						for (var i = 0; i < pairs.length; i++) {
							var pair = pairs[i];
				...
				Demo.sprites = function() {
					var _world = _engine.world, offset = 10, options = {
						isStatic : true,
						render : {
							visible : false
						}
					...
					// these static walls will not be rendered in this sprites
					// example, see options
					World.add(_world, [ Bodies.rectangle(400, -offset, 800.5 + 2 * offset, 50.5, options), Bodies.rectangle(400, 600 + offset, 800.5 + 2 * offset, 50.5, options), Bodies.rectangle(800 + offset, 300, 50.5, 600.5 + 2 * offset, options), Bodies.rectangle(-offset, 300, 50.5, 600.5 + 2 * offset, options) ]);
						...
						if (Math.random() > 0.35) {
							return Bodies.rectangle(x, y, 64, 64, {
								render : {
									strokeStyle : '#ffffff',
									sprite : {
										texture : 'assets/js/apps/gamesDemo/img/box.png'
									}
								}
							});
						} else {
							return Bodies.circle(x, y, 46, {
								...
								render : {
									sprite : {
										texture : 'assets/js/apps/gamesDemo/img/ball.png'
									}
								}
							});
						}
					...
					var renderOptions = _engine.render.options;
					renderOptions.background = 'assets/js/apps/gamesDemo/img/wall-bg.jpg';
					renderOptions.showAngleIndicator = false;
					renderOptions.wireframes = false;
				};
				...
				Demo.raycasting = function() {
					...
					_sceneEvents.push(Events.on(_engine, 'afterRender', function() {
						var mouse = _engine.input.mouse, context = _engine.render.context, bodies = Composite.allBodies(_engine.world), startPoint = {
							x : 400,
							y : 100
				...
				Demo.initControls = function() {
					var demoSelect = document.getElementById('demo-select'), demoReset = document.getElementById('demo-reset');

					// create a Matter.Gui
					if (!_isMobile && Gui) {
						_gui = Gui.create(_engine);

						// need to add mouse constraint back in after gui clear
						// or load is pressed
					...
					if (_isMobile) {
						...
						var fullscreenChange = function() {
							...
							// delay fullscreen styles until fullscreen has
							// finished changing
							setTimeout(function() {
				...
				Demo.reset = function() {
					...
					// reset mouse offset and scale (only required for
					// Demo.views)
					...
					var offset = 5;
					World.add(_world, [ Bodies.rectangle(400, -offset, 800.5 + 2 * offset, 50.5, {
						isStatic : true
