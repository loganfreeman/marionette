define(['pixi', 'async', 'createjs', 'tweenjs', 'vendor/easystar-0.1.5.min', 'vendor/bootstrap/bootstrap-switch'], function(PIXI, async, createjs) {
	var EasyStarDemo = function(width, height) {
		...
		var subtextures = [];
		var imageLoader = new PIXI.ImageLoader('assets/js/apps/gamesDemo/img/bg.png');
		var spriteSheetLoader = new PIXI.SpriteSheetLoader('assets/js/apps/gamesDemo/img/sprites.json');
		...
		var createGrid = function(width, height) {
			...
			for (var y = 0; y < height; y++) {
				...
				for (var x = 0; x < width; x++) {
					if (x >= BOARD_PADDING && x <= width - BOARD_PADDING - 1) {
						if (Math.random() > .4) {
							grid[y][x] = 1;
						} else {
							grid[y][x] = 0;
		...
		var createBoardSprite = function(texture, container, posX, posY) {
			...
			createjs.Tween.get(sprite.position).to({
				x : Math.floor(posX + offsetX / 2),
				y : Math.floor(posY + offsetY / 2)
			}, 200); // .call(handleComplete);
			createjs.Tween.get(sprite).to({
				alpha : 1
			}, 300); // .call(handleComplete);
			return sprite;
		};
		...
		var findPathForHero = function(hero, callback) {
			easystar.findPath(hero.x, hero.y, hero.destinationX, hero.destinationY, function(path) {
				...
				var id = setInterval(function() {
					...
					createjs.Tween.get(hero.sprite.position).to({
						x : destinationX,
						y : destinationY
					}, 100); // .call(handleComplete);
					path.splice(0, 1);
