define(['vendor/coquette'], function() {
	var GOLDEN_RATIO = 1.61803398875;

	var SpinningShapesGame = function() {
		var autoFocus = false;
		this.c = new Coquette(this, "spinning-shapes-canvas", 500, 500 / GOLDEN_RATIO, "white", autoFocus);
		this.dragger = new Dragger(this.c); // controls dragging of shapes with
											// mouse
	};
	...
	Rectangle.prototype = {
		...
		draw : function(ctx) {
			...
			ctx.strokeRect(this.center.x - this.size.x / 2, this.center.y - this.size.y / 2, this.size.x, this.size.y);
		},
	...
	var randomDirection = function() {
		return Coquette.Collider.Maths.unitVector({
			x : Math.random() - .5,
			y : Math.random() - .5
		});
	};
	...
	var movingOnscreenVec = function(dirFromCenter) {
		return {
			x : -dirFromCenter.x * 3 * Math.random(),
			y : -dirFromCenter.y * 3 * Math.random()
		}
	};
	...
	var isOutOfView = function(obj, viewSize, viewCenter) {
		return Coquette.Collider.Maths.distance(obj.center, viewCenter) > Math.max(viewSize.x, viewSize.y);
	};
	...
	var makeCurrentCollidersCountable = {
		colliderCount : 0, // number of other shapes currently touching this
							// shape
