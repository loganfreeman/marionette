define([ 'd3.chart' ], function() {
	d3.chart("Hybrid", {
		...
		initialize : function() {
			...
			bc.width(barWidth);
			bc.height(barHeight);
			bc.base.attr("transform", "translate(" + (this.base.attr("width") / 2) + "," + (this.base.attr("height") / 2 + this.radius - barHeight) + ")");
		...
		transformBars : function() {
			...
			// overridden (otherwise, using a chart as a mixin will break that
			// chart),
			// but there needs to be a way to access the higher-level chart from
			// an
