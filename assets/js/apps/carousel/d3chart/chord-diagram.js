define([ 'd3.chart' ], function() {
	d3.chart("Chord", {
		...
		initialize : function(options) {
			...
			var base = chart.base.append("g").attr("transform", "translate(" + this.width() / 2 + "," + this.height() / 2 + ")");
			...
			function onEnterHandles() {
				this.style("fill", function(d) {
					...
					return fill(d.index);
				}).attr("d", d3.svg.arc().innerRadius(chart.innerRadius).outerRadius(chart.outerRadius)).on("mouseover", fade(.1)).on("mouseout", fade(1));
			}

			function onEnterTicks() {
				this.attr("transform", function(d) {
					return "rotate(" + (d.angle * 180 / Math.PI - 90) + ")" + "translate(" + chart.outerRadius + ",0)";
				});

				this.append("line").attr("x1", 1).attr("y1", 0).attr("x2", 5).attr("y2", 0).style("stroke", "#000");

				this.append("text").attr("x", 8).attr("dy", ".35em").attr("transform", function(d) {
			...
			function onEnterChords() {
				this.attr("d", d3.svg.chord().radius(chart.innerRadius)).style("fill", function(d) {
			...
			this.layer("ticks", base.append("g"), {
				dataBind : function(chord) {
					return this.selectAll("g").data(chord.groups).enter().append("g").selectAll("g").data(groupTicks);
				},
				insert : function() {
			...
			function fade(opacity) {
				return function(g, i) {
					chart.base.selectAll(".chord path").filter(function(d) {
		...
		transform : function(data) {
			return d3.layout.chord().padding(.05).sortSubgroups(d3.descending).matrix(data);
		}
		...
		initialize : function(options) {
			...
			this.layer("ticks").on("enter", function() {
				this.each(function(data, idx, group) {
					d3.select(this).style("font-weight", "bold").attr("fill", chart.colors[group]);
				});
			});
