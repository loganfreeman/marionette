define([ 'app', 'stache!apps/chat/main', 'css!apps/chat/style', 'jScrollPane', 'jquery.defaultText' ], function(App, viewTpl) {
	App.module('chat', function(chat, App, Backbone, Marionette, $, _) {
		...
		var chat = {
			...
			init : function() {
				...
				$('#chatTopBar').on('click', 'a.logoutButton', function() {
			...
			render : function(template, params) {
				...
				switch (template) {
				case 'loginTopBar':
					arr = [ '<span><img src="', params.gravatar, '" width="23" height="23" />', '<span class="name">', params.name, '</span><a href="" class="logoutButton rounded">Logout</a></span>' ];
					break;

				case 'chatLine':
					arr = [ '<div class="chat chat-', params.id, ' rounded"><span class="gravatar"><img src="', params.gravatar, '" width="23" height="23" onload="this.style.visibility=\'visible\'" />', '</span><span class="author">', params.author, ':</span><span class="text">', params.text, '</span><span class="time">', params.time, '</span></div>' ];
					break;
			...
			addChatLine : function(params) {
				...
				if (params.time) {
					...
					// PHP returns the time in UTC (GMT). We use it to feed the
					// date
					// object and later output it in the user's timezone.
					// JavaScript
		...
		chat.MainView = Marionette.ItemView.extend({
			template : viewTpl,
			onShow: function(){
				chat.init();
			}
		...
		chat.controller = {
			show : function() {
				App.navigate("chat");
				App.mainRegion.show(new chat.MainView());
			}
		}
		...
		chat.Router = Marionette.AppRouter.extend({
			appRoutes : {
				"chat" : "show"
			}
		});
