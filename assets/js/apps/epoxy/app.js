define([ 'app',
         'stache!apps/epoxy/main',
         'css!apps/epoxy/style', 'backbone.epoxy'], function(App, viewTpl) {
	App.module('epoxyDemo', function(epoxyDemo, App, Backbone, Marionette, $, _) {

		epoxyDemo.MainView = Marionette.ItemView.extend({
			template : viewTpl,
			onShow: function(){
		...
		epoxyDemo.controller = {
			show : function() {
				App.navigate("epoxyDemo");
				App.mainRegion.show(new epoxyDemo.MainView());
			}
		}

		App.on("epoxyDemo:show", function() {
			epoxyDemo.controller.show();
		});

		epoxyDemo.Router = Marionette.AppRouter.extend({
			appRoutes : {
				"epoxyDemo" : "show"
			}
		});
		...
		App.addInitializer(function() {
			new epoxyDemo.Router({
				controller : epoxyDemo.controller
			});
		});
	});
	return App.epoxyDemo.controller;
})