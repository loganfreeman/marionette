define([],
		function() {
			/**
			 * Usage: connect two shapes parameters: source shape [or connection
			 * for redrawing], target shape, style with { fg : linecolor, bg :
			 * background color, directed: boolean } returns: connection { draw =
			 * function() }
			 */
			Raphael.fn.connection = function(obj1, obj2, style) {
				...
				var edge = {/*
					...
					draw : function() {
						...
						var off2 = 0;
						/*
						 * coordinates for potential connection coordinates
						 * from/to the objects
						 */
						var p = [ {
							x : bb1.x + bb1.width / 2,
						...
						/*
						 * distances between objects and according coordinates
						 * connection
						 */
						var d = {}, dis = [];

						/*
						 * find out the best connection coordinates by trying
						 * all possible ways
						 */
						...
						/* bezier path */
						var x1 = p[res[0]].x, y1 = p[res[0]].y, x4 = p[res[1]].x, y4 = p[res[1]].y, dx = Math.max(Math.abs(x1 - x4) / 2, 10), dy = Math.max(Math.abs(y1 - y4) / 2, 10), x2 = [ x1, x1, x1 - dx, x1 + dx ][res[0]].toFixed(3), y2 = [ y1 - dy, y1 + dy, y1, y1 ][res[0]].toFixed(3), x3 = [ 0, 0, 0, 0, x4, x4, x4 - dx, x4 + dx ][res[1]].toFixed(3), y3 = [ 0, 0, 0, 0, y1 + dy, y1 - dy, y4,
								y4 ][res[1]].toFixed(3);
						...
						if (style && style.directed) {
							...
							/*
							 * calculate array coordinates (two lines orthogonal
							 * to the path vector)
							 */
							var arr = [ {
						...
						/*
						 * function to be used for moving existent path(s), e.g.
						 * animate() or attr()
						 */
						var move = "attr";
						/* applying path(s) */
						...
						/* setting label */
						style && style.label && (edge.label && edge.label.attr({
							x : (x1 + x4) / 2,
							y : (y1 + y4) / 2
						}) || (edge.label = selfRef.text((x1 + x4) / 2, (y1 + y4) / 2, style.label).attr({
							fill : "#000",
