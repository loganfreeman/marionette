define(['arbor'], function(arbor){
    ...
    var that = {
      init:function(system){
        ...
        particleSystem.screen({padding:[100, 60, 60, 60], // leave some space
															// at the bottom for
															// the param sliders
                              step:.02}) // have the ‘camera’ zoom somewhat
											// slowly as the graph unfolds
      ...
      resize:function(){
        var w = $(window).width(),
            h = $(window).height();
        canvas.width = w; canvas.height = h // resize the canvas element to fill
											// the screen
        particleSystem.screenSize(w,h) // inform the system so it can map
										// coords for us
        that.redraw()
      },
  ...
  var Maps = function(elt){
    ...
    sys.renderer = Renderer("#viewport") // our newly created renderer will
    ...
    var that = {
      ...
      selectMap:function(map_id){
        $.getJSON("assets/js/apps/data/maps/"+map_id+".json",function(data){
