define(['jquery', 'jquery.event.drag-2.2', 'jquery.event.drop-2.2', 'vendor/slickgrid/slick.core',
        'vendor/slickgrid/slick.formatters',
        'vendor/slickgrid/slick.editors', 
        'css!vendor/slickgrid/slick.grid'], function(jQuery) {
	/**
	 * @license (c) 2009-2013 Michael Leibman
	 ...
	 * SlickGrid v2.2
	 * 
	 * NOTES: Cell/row DOM manipulations are done directly bypassing jQuery's
	 * DOM manipulation methods. This increases the speed dramatically, but can
	 * only be done safely because there are no event handlers or data
	 * associated with any cell/row DOM nodes. Cell editors must make sure they
	 * implement .destroy() and do proper cleanup.
	 */
	...
	(function($) {
		...
		function SlickGrid(container, data, columns, options) {
			...
			var rowNodeFromLastMouseWheelEvent; // this node must not be deleted
												// while inertial scrolling
			var zombieRowNodeFromLastMouseWheelEvent; // node that was hidden
														// instead of getting
														// deleted
			...
			function finishInitialization() {
				if (!initialized) {
					...
					// header columns and cells may have different
					// padding/border skewing width calculations (box-sizing,
					// hello?)
					...
					// disabled
					// with the exception of input and textarea elements
					// (selection must
					// be enabled there so that editors work as expected); note
					// that
					// selection in grid cells (grid body) is already
					// unavailable in
					// all browsers except IE
					disableSelection($headers); // disable all text selection in
												// header (including input and
												// textarea)

					if (!options.enableTextSelectionOnCells) {
						// disable text selection in grid cells except in input
						// and textarea elements
						// (this is IE-specific, because selectstart event will
						// only fire in IE)
			...
			function getMaxSupportedCssHeight() {
				...
				// FF reports the height back but still renders blank after ~6M
				// px
			...
			function getViewportHeight() {
				return parseFloat($.css($container[0], "height", true)) - parseFloat($.css($container[0], "paddingTop", true)) - parseFloat($.css($container[0], "paddingBottom", true)) - parseFloat($.css($headerScroller[0], "height")) - getVBoxDelta($headerScroller) - (options.showTopPanel ? options.topPanelHeight + getVBoxDelta($topPanelScroller) : 0)
			...
			function updateRowCount() {
				...
				// with autoHeight, we do not need to accommodate the vertical
				// scroll bar
				...
				// this helps avoid redundant calls to .removeRow() when the
				// size of the data decreased by thousands of rows
			...
			function cleanUpCells(range, row) {
				...
				for ( var i in cacheEntry.cellNodesByColumnIdx) {
					...
					// This is a string, so it needs to be cast back to a
					// number.
					i = i | 0;
			...
			function cleanUpAndRenderCells(range) {
				...
				for (var row = range.top, btm = range.bottom; row <= btm; row++) {
					...
					// cellRenderQueue populated in renderRows() needs to be
					// cleared first
					ensureCellNodesInRowsCache(row);
					...
					// TODO: shorten this loop (index? heuristics? binary
					// search?)
					for (var i = 0, ii = columns.length; i < ii; i++) {
			...
			function renderRows(range) {
				...
				for (var i = range.top, ii = range.bottom; i <= ii; i++) {
					...
					rowsCache[i] = {
						...
						// Can also be used for checking whether a cell has been
						// rendered.
						"cellColSpans" : [],
						...
						// Column indices of cell nodes that have been rendered,
						// but not yet indexed in
						// cellNodesByColumnIdx. These are in the same order as
						// cell nodes added at the
						// end of the row.
						"cellRenderQueue" : []
					};
			...
			function handleDragInit(e, dd) {
				...
				// if nobody claims to be handling drag'n'drop by stopping
			...
			function handleKeyDown(e) {
					...
					if (!e.shiftKey && !e.altKey && !e.ctrlKey) {
						if (e.which == 27) {
							if (!getEditorLock().isActive()) {
								return; // no editing mode to cancel, allow
										// bubbling and default processing (exit
				...
				if (handled) {
					...
					e.preventDefault();
					try {
						e.originalEvent.keyCode = 0; // prevent default
														// behaviour for special
														// keys in IE browsers
														// (F3, F5, etc.)
					}
					// ignore exceptions - setting the original event's keycode
					// throws access denied exception for "Ctrl"
					// (hitting control key only, nothing else), "Shift" (maybe
					// others)
					catch (error) {
					}
				}
			...
			function handleClick(e) {
				if (!currentEditor) {
					// if this click resulted in some cell child node getting
					// focus,
					// don't steal it back - keyboard events will still bubble
					// up
					// IE9+ seems to default DIVs to tabIndex=0 instead of -1,
					// so check for cell clicks directly.
			...
			function makeActiveCellNormal() {
				...
				// if there previously was text selected on a page (such as
				// selected text in the edit cell just removed),
			 ...
			 * @return {boolean} Whether navigation resulted in a change of
			 *         active cell.
			 */
				...
				if (currentEditor) {
					if (currentEditor.isValueChanged()) {
						...
						if (validationResults.valid) {
							...
							// check whether the lock has been re-acquired by
							// event handlers
							return !getEditorLock().isActive();
						} else {
							// Re-add the CSS class to trigger transitions, if
							// any.
