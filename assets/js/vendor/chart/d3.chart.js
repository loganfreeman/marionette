define([ 'd3.v3' ], function(d3) {
	 ...
	 * exposed to d3.chart users. Instead, its instance methods are mixed in to
	 * the `base` selection it describes; users interact with the instance via
	 * these bound methods.
	 * 
	 * @private
	 * @constructor
	 * 
	 * @param {d3.selection}
	 *            base The containing DOM node for the layer.
	 */
	...
	/**
	 * Invoked by {@link Layer#draw} to join data with this layer's DOM nodes.
	 * This implementation is "virtual"--it *must* be overridden by Layer
	 * instances.
	 * 
	 * @param {Array}
	 *            data Value passed to {@link Layer#draw}
	 */
	 ...
	 * layer's `base`. This implementation is "virtual"--it *must* be overridden
	 * by Layer instances.
	 */
	 ...
	 * 
	 * @param {String}
	 *            eventName Identifier for the lifecycle event for which to
	 *            subscribe.
	 * @param {Function}
	...
	/**
	 * Unsubscribe the specified handler from the specified event. If no handler
	 * is supplied, remove *all* handlers from the event.
	 * 
	 * @param {String}
	 *            eventName Identifier for event from which to remove
	 *            unsubscribe
	 * @param {Function}
	 ...
	 * handlers (as attached via {@link Layer#on}) with the lifecycle
	 * selections.
	 *  - update - update:transition - enter - enter:transition - exit -
	 * exit:transition
	 * 
	 * @param {Array}
	 ...
	 * 
	 * @param {Object}
	 *            [options] Options to be forwarded to {@link Layer|the Layer
	 *            constructor}
	...
	/**
	 * Call the {@Chart#initialize} method up the inheritance chain, starting
	 * with the base class and continuing "downward".
	 * 
	 * @private
	 */
	 ...
	 * instance and continuing "upward". The result of each transformation
	 * should be supplied as input to the next.
	 * 
	 * @private
	 */
	var transformCascade = function(instance, data) {
		...
		// attached directly to a chart instance. Ensure that this transform
		// takes
		...
		// Do not invoke the `transform` method on classes further up the
		// prototype
		// chain (yet).
	 ...
	 * 
	 * @param {d3.selection}
	 *            selection The chart's "base" DOM node. This should contain any
	 *            nodes that the chart generates.
	 * @param {mixed}
	 *            chartOptions A value for controlling how the chart should be
	 *            created. This value will be forwarded to
	 *            {@link Chart#initialize}, so charts may define additional
	 *            properties for consumers to modify their behavior during
	 *            initialization.
	 * 
	 * @constructor
	 */
	...
	/**
	 * Set up a chart instance. This method is intended to be overridden by
	 * Charts authored with this library. It will be invoked with a single
	 * argument: the `options` value supplied to the {@link Chart|constructor}.
	 * 
	 ...
	 * with the "oldest" ancestor (see the private {@link initCascade} function
	 * for more details).
	 */
	 ...
	 * `name`.
	 * 
	 * If all three arguments are specified, initialize a new {@link Layer}
	 * using the specified `selection` as a base passing along the specified
	 * `options`.
	 * 
	 * The {@link Layer.draw} method of attached layers will be invoked whenever
	 * this chart's {@link Chart#draw} is invoked and will receive the data
	 * (optionally modified by the chart's {@link Chart#transform} method.
	 * 
	 * @param {String}
	 *            name Name of the layer to attach or retrieve.
	 * @param {d3.selection|Layer}
	 *            [selection] The layer's base or a previously-created
	 *            {@link Layer}.
	 * @param {Object}
	 *            [options] Options to be forwarded to {@link Layer|the Layer
	 *            constructor}
	...
	/**
	 * Register or retrieve an "attachment" Chart. The "attachment" chart's
	 * `draw` method will be invoked whenever the containing chart's `draw`
	 * method is invoked.
	 * 
	 * @param {String}
	 *            attachmentName Name of the attachment
	 * @param {Chart}
	 *            [chart] d3.chart to register as a mix in of this chart. When
	...
	/**
	 * Update the chart's representation in the DOM, drawing all of its layers
	 * and any "attachment" charts (as attached via {@link Chart#attach}).
	 * 
	 * @param {Object}
	 *            data Data to pass to the {@link Layer#draw|draw method} of
	...
	/**
	 * Function invoked with the context specified when the handler was bound
	 * (via {@link Chart#on} {@link Chart#once}).
	 * 
	 * @callback ChartEventHandler
	 * @param {...*}
	 *            arguments Invoked with the arguments passed to {@link
	 *            Chart#trigger}
	 */

	/**
	 * Subscribe a callback function to an event triggered on the chart. See
	 * {@link Chart#once} to subscribe a callback function to an event for one
	 * occurence.
	 * 
	 * @param {String}
	 *            name Name of the event
	 * @param {ChartEventHandler}
	 *            callback Function to be invoked when the event occurs
	 * @param {Object}
	 *            [context] Value to set as `this` when invoking the `callback`.
	 ...
	 * function will be invoked at the next occurance of the event and
	 * immediately unsubscribed. See {@link Chart#on} to subscribe a callback
	 * function to an event indefinitely.
	 * 
	 * @param {String}
	 *            name Name of the event
	 * @param {ChartEventHandler}
	 ...
	 * chart. When no arguments are specified, *all* handlers will be
	 * unsubscribed. When only a `name` is specified, all handlers subscribed to
	 * that event will be unsubscribed. When a `name` and `callback` are
	 * specified, only that function will be unsubscribed from that event. When
	 * a `name` and `context` are specified (but `callback` is omitted), all
	 * events bound to the given event with the given context will be
	 * unsubscribed.
	 * 
	 * @param {String}
	 ...
	 * @param {Object}
	 *            [context] Contexts to be unsubscribe
	 * 
	 * @returns {Chart} A reference to this chart (chainable).
	 */
	 ...
	 * @param {...*}
	 *            arguments Values with which to invoke the registered
	 *            callbacks.
	 * 
	 * @returns {Chart} A reference to this chart (chainable).
	 */
	...
	/**
	 * Create a new {@link Chart} constructor with the provided options acting
	 * as "overrides" for the default chart instance methods. Allows for basic
	 ...
	 * 
	 * @param {String}
	 *            name Identifier for the new Chart constructor.
	 * @param {Object}
	 *            protoProps Properties to set on the new chart's prototype.
	 * @param {Object}
	 *            staticProps Properties to set on the chart constructor itself.
	 * 
	 ...
	 * 
	 * @param {String}
	 *            name If no other arguments are specified, return the
	 *            previously-created chart with this name.
	 * @param {Object}
	 *            protoProps If specified, this value will be forwarded to
	 *            {@link Chart.extend} and used to create a new chart.
	 * @param {Object}
	 *            staticProps If specified, this value will be forwarded to
	 *            {@link Chart.extend} and used to create a new chart.
	 */
	...
	/**
	 * Instantiate a chart or return the chart that the current selection
	 * belongs to.
	 * 
	 * @static
	 * 
	 * @param {String}
	 *            [chartName] The name of the chart to instantiate. If the name
	 *            is unspecified, this method will return the chart that the
	 *            current selection belongs to.
	 * @param {mixed}
	 *            options The options to use when instantiated the new chart.
