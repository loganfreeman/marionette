define(function() {
	var _registry = {}, _scopes = {}, _attachments = {}, _extensions = {}, _debugging = [], _debugAll = false, _framework, _dom;
	...
	// Either matches the chosen JS framework to the passed guess, or returns
	// the
	// current framework.
	...
	Eve = {
		...
		setFramework : function(fw) {
			...
			if (_framework == 'jquery')
				$ = jQuery; // No-conflict compat.
			if (_framework == 'zepto')
				$ = Zepto; // No-conflict compat.
		},
		...
		attach : function(moduleName, namespace) {
			...
			fun = function() {
				_registry[moduleName].apply(this, args.slice(2));
			}
			dbug(moduleName, "attached to " + namespace);
			// We're delegating off the window, so there's no need to reattach
			// for
	...
	var Scope = {
		...
		listen : function(selector, event, handler) {
			...
			// If listen is happening in the context of a triggered event
			// handler,
			// we only want to delegate to the current event namespace.
			var scope = (this.event) ? this.find() : document.body;

			var name = this.name, sel = (this.namespace + ' ' + selector).trim(), obj = {};
			for ( var k in this)
			...
			// JavaScript framework development is so much easier when you let
			// some
