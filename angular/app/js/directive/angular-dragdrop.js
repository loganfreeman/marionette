define([ 'angular', 'jquery', 'jquery-ui' ], function(angular, $) {
	...
	var jqyoui = angular.module('ngDragDrop', []).service('ngDragDropService', [ '$timeout', '$parse', function($timeout, $parse) {
		this.callEventCallback = function(scope, callbackName, event, ui) {
			...
			// call either $scoped method i.e. $scope.dropCallback or
			// constructor's method i.e. this.dropCallback
			scope.$apply((scope[callback] || scope[constructor][callback]).apply(scope, args));

			function extract(callbackName) {
				var atStartBracket = callbackName.indexOf('(') !== -1 ? callbackName.indexOf('(') : callbackName.length, atEndBracket = callbackName.lastIndexOf(')') !== -1 ? callbackName.lastIndexOf(')') : callbackName.length, args = callbackName.substring(atStartBracket + 1, atEndBracket), // matching
																																																																										// function
																																																																										// arguments
																																																																										// inside
																																																																										// brackets
				constructor = callbackName.match(/^[^.]+.\s*/)[0].slice(0, -1); // matching
																				// a
																				// string
																				// upto
																				// a
			...
			if (dragSettings.animate === true) {
				...
				this.move($droppableDraggable.length > 0 && !dropSettings.multiple ? $droppableDraggable : [], $draggable.parent('[jqyoui-droppable],[data-jqyoui-droppable]'), jqyoui.startXY, 'fast', dropSettings, angular.bind(this, function() {
					$timeout(angular.bind(this, function() {
						// Do not move this into move() to avoid flickering
						// issue
						$draggable.css({
							'position' : 'relative',
							'left' : '',
						...
						// Angular v1.2 uses ng-hide to hide an element not
						// display property
						// so we've to manually remove display:none set in
						// this.move()
		...
		this.mutateDraggable = function(scope, dropSettings, dragSettings, dragModel, dropModel, dropItem, $draggable) {
			...
			if (dragSettings && dragSettings.placeholder) {
				...
				if (angular.isArray(dragModelValue)) {
					...
					// Fix: LIST(object) to LIST(array) - model does not get
					// updated using just scope[dragModel] = {...}
