define([ 'angular' ], function(angular) {
	...
	var uiModual = angular.module('ui', [ 'ui.filters', 'ui.directives', 'ui.config' ]);

	/*
	 * jQuery UI Sortable plugin wrapper
	 * 
	 * @param [ui-sortable] {object} Options to pass to $.fn.sortable() merged
	 * onto ui.config
	 */
	...
	/**
	 * General-purpose jQuery wrapper. Simply pass the plugin name as the
	 * expression.
	 * 
	 * It is possible to specify a default set of parameters for each jQuery
	 * plugin. Under the jq key, namespace each plugin by that which will be
	 * passed to ui-jq. Unfortunately, at this time you can only pre-define the
	 * first parameter.
	 * 
	 * @example { jq : { datepicker : { showOn:'click' } } }
	 * 
	 * @param ui-jq
	 *            {string} The $elm.[pluginName]() to call.
	 * @param [ui-options]
	 *            {mixed} Expression to be evaluated and passed as options to
	 *            the function Multiple parameters can be separated by commas
	 *            Set {ngChange:false} to disable passthrough support for change
	 *            events ( since angular watches 'input' events, not 'change'
	 *            events )
	 * 
	 * @example <input ui-jq="datepicker"
	 *          ui-options="{showOn:'click'},secondParameter,thirdParameter">
	 */
	 ...
	 * Angular Pass an object with keynames for events to ui-event Allows $event
	 * object and $params object to be passed
	 * 
	 * @example <input ui-event="{ focus : 'counter++', blur : 'someCallback()'
	 *          }">
	 * @example <input ui-event="{ myCustomEvent : 'myEventHandler($event,
	 *          $params)'}">
	 * 
	 * @param ui-event
	 *            {string|object literal} The event to bind to as a string or a
	 *            hash of events with their callbacks
	 */
	...
	angular.module('ui.directives').directive('uiMask', [ function() {
		return {
			...
			link : function($scope, element, attrs, controller) {
				...
				/*
				 * Add a parser that extracts the masked value into the model
				 * but only if the mask is valid
				 */
				controller.$parsers.push(function(value) {
					// the second check (or) is only needed due to the fact that
					// element.isMaskValid() will keep returning undefined
	...
	angular.module('ui.directives').directive('uiModal', [ '$timeout', function($timeout) {
		return {
			...
			link : function(scope, elm, attrs, model) {
				...
				// If bootstrap animations are enabled, listen to 'shown' and
				// 'hidden' events
	...
	(function() {
		...
		// Setup map events from a google map object to trigger on a given
		// element too,
		// then we just use ui-event to catch events from an element
		function bindMapEvents(scope, eventsStr, googleObject, element) {
			angular.forEach(eventsStr.split(' '), function(eventName) {
				// Prefix all googlemap events with 'map-', so eg 'click'
				// for the googlemap doesn't interfere with a normal 'click'
				// event
				var $event = {
					type : 'map-' + eventName
				};
				google.maps.event.addListener(googleObject, eventName, function(evt) {
					...
					// We create an $apply if it isn't happening. we need better
					// support for this
					// We don't want to use timeout because tons of these events
					// fire at once,
		...
		app.directive('uiMapInfoWindow', [ 'ui.config', '$parse', '$compile', function(uiConfig, $parse, $compile) {
			...
			return {
				link : function(scope, elm, attrs) {
					...
					/*
					 * The info window's contents dont' need to be on the dom
					 * anymore, google maps has them stored. So we just replace
					 * the infowindow element with an empty div. (we don't just
					 * straight remove it from the dom because straight removing
					 * things from the dom can mess up angular)
					 */
					elm.replaceWith('<div></div>');

					// Decorate infoWindow.open to $compile contents before
					// opening
		 ...
		 * <ui-map-marker="myMarker"> will $watch 'myMarker' and each time it
		 * changes, it will hook up myMarker's events to the directive dom
		 * element. Then ui-event will be able to catch all of myMarker's
		 * events. Super simple.
		 */
	 ...
	 * 
	 * @param hash
	 *            {mixed} keyBindings Can be an object or string where
	 *            keybinding expression of keys or keys combinations and
	 *            AngularJS Exspressions are set. Object syntax: "{ keys1:
	 *            expression1 [, keys2: expression2 [ , ... ]]}". String syntax:
	 *            ""expression1 on keys1 [ and expression2 on keys2 [ and ...
	 *            ]]"". Expression is an AngularJS Expression, and key(s) are
	 *            dash-separated combinations of keys and modifiers (one or
	 *            many, if any. Order does not matter). Supported modifiers are
	 *            'ctrl', 'shift', 'alt' and key can be used either via its
	 *            keyCode (13 for Return) or name. Named keys are 'backspace',
	 *            'tab', 'enter', 'esc', 'space', 'pageup', 'pagedown', 'end',
	 *            'home', 'left', 'up', 'right', 'down', 'insert', 'delete'.
	 * @example <input ui-keypress="{enter:'x = 1', 'ctrl-shift-space':'foo()',
	 *          'shift-13':'bar()'}" /> <input ui-keypress="foo = 2 on ctrl-13
	 *          and bar('hello') on shift-esc" />
	 */
	...
	/**
	 * Animates the injection of new DOM elements by simply creating the DOM
	 * with a class and then immediately removing it Animations must be done
	 * using CSS3 transitions, but provide excellent flexibility
	 * 
	 * @todo Add proper support for animating out
	 * @param [options]
	 *            {mixed} Can be an object with multiple options, or a string
	 *            with the animation class class {string} the CSS class(es) to
	 *            use. For example, 'ui-hide' might be an excellent alternative
	 *            class.
	 * @example
	 *            <li ng-repeat="item in items" ui-animate=" 'ui-hide' ">{{item}}</li>
	 */
	angular.module('ui.directives').directive('uiAnimate', [ 'ui.config', '$timeout', function(uiConfig, $timeout) {
		...
		return {
			restrict : 'A', // supports using directive as element, attribute
							// and class
	 ...
	 * 
	 * @AJAX Mode - When in this mode, your value will be an object (or array of
	 *       objects) of the data used by Select2 This change is so that you do
	 *       not have to do an additional query yourself on top of Select2's own
	 *       query
	 * @params [options] {object} The configuration options passed to
	 *         $.fn.select2(). Refer to the documentation
	 */
					...
					if (controller) {
						...
						if (watch) {
							scope.$watch(watch, function(newVal, oldVal, scope) {
								if (!newVal)
									return;
								// Delayed so that the options have time to be
								// rendered
								setTimeout(function() {
									elm.select2('val', controller.$viewValue);
									// Refresh angular to remove the superfluous
									// option
									elm.trigger('change');
								});
							});
						...
						if (!isSelect) {
							// Set the view and model value and update the
							// angular template manually for the ajax/multiple
							// select2.
	...
	angular.module('ui.directives').directive('uiCodemirror', [ 'ui.config', '$parse', function(uiConfig, $parse) {
		...
		return {
			...
			link : function(scope, elm, attrs, ngModel) {
				...
				var codemirror;
				// This is the method that we use to get the value of the
				// ui-codemirror attribute expression.
				var uiCodemirrorGet = $parse(attrs.uiCodemirror);
				// This method will be called whenever the code mirror widget
				// content changes
				var onChangeHandler = function(ed) {
					// We only update the model if the value has changed - this
					// helps get around a little problem where $render triggers
					// a change despite already being inside a $apply loop.
				...
				// Create and wire up a new code mirror widget (unwiring a
				// previous one if necessary)
				var updateCodeMirror = function(options) {
					// Merge together the options from the uiConfig and the
					// attribute itself with the onChange event above.
					options = angular.extend({}, options, uiConfig.codemirror);

					// We actually want to run both handlers if the user has
					// provided their own onChange handler.
					...
					// If there is a codemirror widget for this element already
					// then we need to unwire if first
					if (codemirror) {
				...
				// with the underlying model, in the case that it is changed by
				// something else.
		...
		return {
			...
			link : function(scope, elm, attrs, ngModel) {
				var expression, options = {
					...
					// Update model when calling setContent (such as from the
					// source editor popup)
					setup : function(ed) {
	 ...
	 * depending on a condition Originally created by @tigbro, for the
			...
			compile : function(element, attr, linker) {
				return function(scope, iterStartElement, attr) {
					...
					scope.$watch(expression, function(newValue) {
						...
						// Note: need to be parent() as jquery cannot trigger
						// events on comments
						// (angular creates a comment node when using
						// transclusion, as ng-repeat does).
	...
	/**
	 * Adds a 'ui-scrollfix' class to the element when the page scrolls past
	 * it's position.
	 * 
	 * @param [offset]
	 *            {int} optional Y-offset to override the detected offset. Takes
	 *            300 (absolute) or -300 or +300 (relative to detected)
	 */
		...
		return {
			link : function(scope, elm, attrs) {
				...
				if (!attrs.uiScrollfix) {
					...
					// http://jsperf.com/indexof-vs-chartat
				...
				angular.element($window).on('scroll.ui-scrollfix', function() {
					// if pageYOffset is defined use it, otherwise use other
					// crap for IE
					var offset;
	...
	/*
	 * AngularJs Fullcalendar Wrapper for the JQuery FullCalendar inspired by
	 * http://arshaw.com/fullcalendar/
	 * 
	 * Basic Angular Calendar Directive that takes in live events as the
	 * ng-model and watches that event array for changes, to update the view
	 * accordingly. Can also take in an event url as a source object(s) and feed
	 * the events per view.
	 * 
	 */
	...
	angular.module('ui.directives').directive('uiCalendar', [ 'ui.config', '$parse', function(uiConfig, $parse) {
		...
		return {
			...
			link : function(scope, elm, $attrs) {
				...
				// update method that is called on load and whenever the events
				// array is changed.
				function update() {
					...
					var expression, options = {
						...
						// Calling the events from the scope through the
						// ng-model binding attribute.
						events : scope.events
					};
					// if attrs have been entered to the directive, then create
					...
					// call fullCalendar from an empty html tag, to keep angular
					// happy.
					elm.html('').fullCalendar(options);
				}
				// on load update call.
				update();
				// watching the length of the array to create a more efficient
				// update process.
	 ...
	 * uiShow Directive
	 * 
	 * Adds a 'ui-show' class to the element instead of display:block Created to
	 * allow tighter control of CSS without bulkier directives
	 * 
	 * @param expression
	 *            {boolean} evaluated expression to determine if the class
	 *            should be added
	 */
	 ...
	 * uiHide Directive
	 * 
	 * Adds a 'ui-hide' class to the element instead of display:block Created to
	 * allow tighter control of CSS without bulkier directives
	 * 
	 * @param expression
	 *            {boolean} evaluated expression to determine if the class
	 ...
	 * uiToggle Directive
	 * 
	 * Adds a class 'ui-show' if true, and a 'ui-hide' if false to the element
	 * instead of display:block/display:none Created to allow tighter control of
	 * CSS without bulkier directives. This also allows you to override the
	 ...
	 * 
	 * @param [ui-date] {object} Options to pass to $.fn.datepicker() merged
	 * onto ui.config
	 */
			...
			link : function(scope, element, attrs, controller) {
				...
				var initDateWidget = function() {
					...
					if (controller) {
						...
						if (opts.onSelect) {
							// Caller has specified onSelect, so call this as
							// well as updating the model
							...
							// No onSelect already specified so just update the
							// model
							opts.onSelect = updateModel;
						}
						// In case the user changes the text directly in the
						// input box
					...
					// If we don't destroy the old one it doesn't update
					// properly when the config changes
					element.datepicker('destroy');
					// Create the new datepicker widget
					element.datepicker(opts);
					// Force a render to override whatever is in the input text
					// box
					controller.$render();
				};
		...
		var directive = {
			...
			link : function(scope, element, attrs, modelCtrl) {
				if (attrs.uiDateFormat === '') {
					...
					// Use the datepicker with the attribute value as the format
					// string to convert to and from a string
					modelCtrl.$formatters.push(function(value) {
						if (angular.isString(value)) {
	 ...
	 * 
	 * @param replace
	 *            {mixed} The tokens to replace depends on type string: all
	 *            instances of $0 will be replaced array: each instance of $0,
	 *            $1, $2 etc. will be placed with each array item in
	 *            corresponding order object: all attributes will be iterated
	 *            through, with :key being replaced with its corresponding value
	 * @return string
	 * 
	 * @example: 'Hello :name, how are you :day'.format({ name:'John',
	 *           day:'Today' })
	 * @example: 'Records $0 to $1 out of $2 total'.format(['10', '20', '3000'])
	 * @example: '$0 agrees to all mentions $0 makes in the event that $0 hits a
	 *           tree while $0 is driving drunk'.format('Bob')
	 */
	...
	/**
	 * Filters out all duplicate items from an array by checking the specified
	 * key
	 * 
	 * @param [key]
	 *            {string} the name of the attribute of each object to compare
	 *            for uniqueness if the key is empty, the entire object will be
	 *            compared if the key === false then no filtering will be
	 *            performed
	 * @return {array}
	 */
	...
	/**
	 * Converts variable-esque naming conventions to something presentational,
	 * capitalized words separated by space.
	 * 
	 * @param {String}
	 *            value The value to be parsed and prettified.
	 * @param {String}
	 *            [inflector] The inflector to use. Default: humanize.
	 * @return {String}
	 * @example {{ 'Here Is my_phoneNumber' | inflector:'humanize' }} => Here Is
	 *          My Phone Number {{ 'Here Is my_phoneNumber' |
	 *          inflector:'underscore' }} => here_is_my_phone_number {{ 'Here Is
	 *          my_phoneNumber' | inflector:'variable' }} => hereIsMyPhoneNumber
	 */
