define([ 'angular', "google-chart-sample" ], function(angular) {
	'use strict';
	...
	angular.module('google-chart-sample').controller("FatChartCtrl", function($scope) {
		...
		$scope.chartSelectionChange = function() {
			...
			if (($scope.chart.type === 'Table' && $scope.chart.data.cols.length === 6 && $scope.chart.options.tooltip.isHtml === true) || ($scope.chart.type != 'Table' && $scope.chart.data.cols.length === 6 && $scope.chart.options.tooltip.isHtml === false)) {
		...
		function fixGoogleChartsBarsBootstrap() {
			// Google charts uses <img height="12px">, which is incompatible
			// with Twitter
			// * bootstrap in responsive mode, which inserts a css rule for: img
			// { height: auto; }.
			// *
			// * The fix is to use inline style width attributes, ie <img
			// style="height: 12px;">.
			// * BUT we can't change the way Google Charts renders its bars. Nor
			// can we change
			// * the Twitter bootstrap CSS and remain future proof.
			// *
			// * Instead, this function can be called after a Google charts
			// render to "fix" the
